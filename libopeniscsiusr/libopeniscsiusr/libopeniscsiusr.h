...
#include "libopeniscsiusr_discovery.h"
...
/* Will load config if exist, or create new one
 * If brand_new is true, existing one will be override with iscsid.conf
 * settings.
 * When port <= 0, default port number 3260 will be used.
 */
__DLL_EXPORT int iscsi_discovery_cfg_new(struct iscsi_context *ctx,
...
/**
 * iscsi_discovery_cfg_free() - Free the memory of 'struct iscsi_discovery'.
 *
 * Free the memory of 'iscsi_discovery' pointer generated by
 * 'iscsi_discovery_cfg_new()'.
 * If provided 'cfg' pointer is NULL.
 *
 * @cfg:
 *	Pointer of 'struct iscsi_discovery_cfg'.
 * Return:
 *	void
 */
__DLL_EXPORT void iscsi_discovery_cfg_free(struct iscsi_discovery_cfg *cfg);

__DLL_EXPORT int iscsi_discovery_cfg_edit(struct iscsi_context *ctx,
					  struct iscsi_discovery_cfg *cfg,
					  const char *name,
					  const char *value);

/* If specified discovery config does not exists, return LIBISCSI_OK and do
 * nothing. */
__DLL_EXPORT int iscsi_discovery_cfg_del(struct iscsi_context *ctx,
					 struct iscsi_discovery_cfg *cfg);

/**
 * iscsi_do_discovery() - Perform a iSCSI discovery.
 *
 * @ctx:
 *	Pointer of 'struct iscsi_context'.
 *	If this pointer is NULL, your program will be terminated by assert.
 * @iface_names:
 *	Pointer of iscsi interface names string array.
 *	The iSCSI interfaces to use for the discovery. When setting to NULL, the
 *	default iSCSI tcp interface will be used.
 * @iface_count:
 *	uint32_t. The size of 'struct iscsi_iface' pointer array.
 * @cfg:
 *	Pointer of 'struct iscsi_discovery_cfg'. The configuration of
 *	discovery to use.
 * @nodes:
 *	Output pointer of 'struct iscsi_node' pointer array. Its memory
 *	should be freed by iscsi_nodes_free().
 ...
 *	Error number could be converted to string by iscsi_strerror().
 */
__DLL_EXPORT int iscsi_do_discovery(struct iscsi_context *ctx,
...
__DLL_EXPORT int iscsi_node_login(struct iscsi_context *ctx,
